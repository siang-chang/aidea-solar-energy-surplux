...
- 逐小時觀測輻射 24 筆：目標日的 24 小時輻射量，取自觀測資料查詢系統，與發電量的相關性最高
...
為了解決上述問題，在訓練模型的時候嘗試將發電廠根據經度或緯度分組，試圖在不大幅增加問題複雜度的前提下[^6]，提升每個群組的資料量。從實驗可以發電，基於經度分組的方法效果較好，並且由於每個發電廠的規模不同，在使用分組方法的時候，需要在模型的輸入變數追加裝置容量，以區分不同案場的資料，具體的輸入變數組成如下：
