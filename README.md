...
對缺失值進行處理，分別計算訓練集與測試集的缺失值比例，並進行處理。
...
- 屬性 Irradiance(kWd/m2) 的缺失值比例不到 1%，且缺值的日期有發電量可以參考，故使用平均陣列轉換率推估輻射後補值。
- 屬性 HourlyTemperature(%) 在訓練集的缺失值比例為 2.19%，使用線性差值進行補值。
- 屬性 HourlyHumidity(%) 在訓練集的缺失值比例為 2.19%，使用線性差值進行補值。
- 屬性 HourlyWindSpeed(%) 在訓練集的缺失值比例為 0.48%，使用線性差值進行補值。

#### 刪除列

- 屬性 Irradiance(kWd/m2) 的缺失值比例不到 1%，直接刪除缺值的列也是一種辦法，將在後續實驗與補值法進行比較。

#### 刪除欄

- 屬性 Temperature(°C) 在測試集的缺失值比例超過 10%，且其中 87% 是連續缺值，補值的效果可能不太理想，考慮到還有氣象局的資料可以用，故直接刪除該欄位。
- 屬性 HourlyCloudAmount 的缺失值比例超過 50%，原因是部分觀測站沒有雲量資料，故直接刪除該欄位。
