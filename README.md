...
| 項目 | 方法                                                                      |  Valid | Public |
| :--- | :------------------------------------------------------------------------ | -----: | -----: |
| 1.A  | 基於經緯度、模組及裝置容量分組，再以預測日的觀測輻射為輸入變數[^8]        | 190.67 |        |
| 1.B  | 基於經緯度、模組及裝置容量分組，再以預測日的觀測及晴空輻射為輸入變數[^9]  | 159.87 |        |
| 1.C  | 基於經緯度、模組及裝置容量分組，再以預測日的觀測及晴空輻射為輸入變數[^10] | 159.27 | 271.40 |
...
| 3    | 將 14 個發電廠依據緯度分組，再以預測日的輻射為輸入變數[^10]           | 119.81 | 276.31 |
| 4    | 將 14 個發電廠依據經度分組，再以預測日的輻射為輸入變數[^10]           | 119.65 | 269.63 |
| 5    | 延續項目 3 基於緯度分組的架構，並在輸入變數追加裝置容量[^11]          | 104.84 |        |
| 6    | 延續項目 4 基於經度分組的架構，並在輸入變數追加裝置容量[^11]          | 101.85 |        |
...
| 項目 | 方法                                                   |  Valid | Public |
| :--- | :----------------------------------------------------- | -----: | -----: |
| 1    | 延續先前最佳實驗，並在輸入變數追加過去 1 日的輻射[^14] | 108.05 | 238.85 |
...
| :--- | :--------------------------------- | -----: |
| 1    | 先前最佳實驗                       | 238.85 |
| 2    | 延續前個實驗，進行模型的超參數調整 | 241.13 |
| 3    | 延續前個實驗，進行模型的超參數調整 | 242.54 |
